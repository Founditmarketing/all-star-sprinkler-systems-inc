...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "allstarsprinklersystems@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "All Star Sprinklers <leads@allstarsprinklers.com>";

  const subject = `New lead from allstarsprinklers.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on allstarsprinklers.com`;
  const html = `
<h2>New lead from the All Star Sprinkler Systems site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on allstarsprinklers.com</p>
`;
